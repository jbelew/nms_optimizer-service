...
from .bonus_calculations import calculate_grid_score, calculate_score_delta
...
async def _handle_ml_opportunity(*args, **kwargs):
    """Handles the ML-based refinement within an opportunity window."""
    # ML placement is not supported in this WASM POC.
    logging.warning("ML refinement is not supported in the WASM build.")
    return None, 0.0


async def _handle_sa_refine_opportunity(
    grid,
    modules,
    ship,
    tech,
    ...
    if num_modules < 6:
        ...
        temp_refined_grid, temp_refined_bonus_local = await simulated_annealing(
            localized_grid,
            ship,
            modules,
            tech,
