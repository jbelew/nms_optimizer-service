...
def calculate_adjacency_bonus(grid: Grid, x: int, y: int) -> float:
    ...
    for dx, dy in directions:
        ...
        if 0 <= nx < grid.width and 0 <= ny < grid.height:
            ...
            # Check if the neighbor is a module of the same technology
            if (
                neighbor["module"] is not None
                and neighbor["tech"] == cell["tech"]
            ):
                adjacency_bonus += 1
    ...
    for row in range(grid.height):
        for col in range(grid.width):
            ...
            if current_cell["module"] is not None and current_cell["tech"] == tech:
                adjacency_bonuses[(col, row)] = calculate_adjacency_bonus(grid, col, row)

    # Set the adjacency bonuses after they have all been calculated
    for (col, row), bonus in adjacency_bonuses.items():
        grid.set_adjacency_bonus(col, row, bonus)
    ...
    for y in range(grid.height):
        for x in range(grid.width):
            ...
            if cell["type"] == "core" and cell["tech"] == tech:
                # Use the pre-calculated adjacency bonus
                adjacency_bonus = cell["adjacency_bonus"]
                ...
                if cell["adjacency"]:
                    total_bonus += adjacency_bonus  # Correctly add the adjacency_bonus
                ...
                grid.set_total(x, y, core_bonus)
                return core_bonus  # Assume only one core module
