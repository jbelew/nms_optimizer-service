...
def train_model(
    ...
    if torchmetrics:
        try:
            ...
            val_accuracy = torchmetrics.Accuracy(task="multiclass", num_classes=num_output_classes, average='macro').to(device)
            val_iou = torchmetrics.JaccardIndex(task="multiclass", num_classes=num_output_classes, average='macro').to(device)
    ...
    if early_stopping_metric == 'val_miou' and not metrics_available:
        print(f"Warning: Cannot monitor 'val_miou' as torchmetrics failed or is not installed. Defaulting to 'val_loss'.")
        ...
        if val_loader:
            ...
            if early_stopping_metric == 'val_miou':
                 if metrics_available:
                     ...
                     print("Warning: Trying to use val_miou for early stopping, but metrics are unavailable. Using val_loss.")
                     current_metric_value = avg_val_loss
...
def run_training_from_files(
    ...
    except KeyError as e: print(f"Error accessing module data: {e}"); return
    except Exception as e: print(f"An unexpected error occurred while getting tech keys: {e}"); return
    if not tech_keys_to_train: print(f"Error: No valid tech keys found for ship '{ship}', category '{tech_category_to_train}'."); return
    ...
    for tech in tech_keys_to_train:
        ...
        if not data_files: print(f"Warning: No data files found matching pattern '{file_pattern}'. Skipping tech '{tech}'."); continue
        print(f"Found {len(data_files)} data files for tech '{tech}'. Loading...")
        all_X_data, all_y_data = [], []
        total_loaded_samples = 0; load_start_time = time.time()
        for filepath in data_files:
            try:
                with np.load(filepath) as npz_file:
                    # Check if keys exist before accessing
                    if "X" not in npz_file or "y" not in npz_file:
                         print(f"Warning: Keys 'X' or 'y' not found in {filepath}. Skipping file."); continue
                    x_batch, y_batch = npz_file["X"], npz_file["y"]
                    # Basic shape validation
                    if len(x_batch.shape) < 3 or len(y_batch.shape) < 3:
                         print(f"Warning: Unexpected array dimensions in {filepath}. Skipping file."); continue
                    if x_batch.shape[1:] != (grid_height, grid_width) or y_batch.shape[1:] != (grid_height, grid_width):
                        print(f"Warning: Shape mismatch in {filepath}. Expected ({grid_height},{grid_width}), got X:{x_batch.shape[1:]}, y:{y_batch.shape[1:]}. Skipping file."); continue
                    if x_batch.shape[0] != y_batch.shape[0]:
                        print(f"Warning: Sample count mismatch in {filepath} (X:{x_batch.shape[0]}, y:{y_batch.shape[0]}). Skipping file."); continue
                    all_X_data.append(x_batch); all_y_data.append(y_batch)
                    total_loaded_samples += x_batch.shape[0]
            except Exception as e: print(f"Error loading data from {filepath}: {e}. Skipping file."); continue
        if not all_X_data: print(f"Warning: No valid data could be loaded for tech '{tech}'. Skipping."); continue
        try:
        ...
        except Exception as e: print(f"Error concatenating data arrays for tech '{tech}': {e}. Skipping."); continue
        # --- End Load Data ---

        # --- Determine num_output_classes ---
        tech_modules = get_tech_modules_for_training(modules, ship, tech)
        if not tech_modules: print(f"Warning: Could not get modules for tech '{tech}'. Skipping."); continue
        num_output_classes = len(tech_modules) + 1
        if num_output_classes <= 1: print(f"Skipping tech '{tech}': Not enough output classes ({num_output_classes})."); continue
        # --- End Determine num_output_classes ---

        # --- 2. Perform Train/Validation Split ---
        X_train_np, X_val_np, y_train_np, y_val_np = None, None, None, None
        val_loader = None # Initialize val_loader to None

        if len(X_data_np) < 2:
            print(f"Warning: Not enough samples ({len(X_data_np)}) for tech '{tech}' to create validation set. Training without validation/early stopping.")
            X_train_np, y_train_np = X_data_np, y_data_np
        elif validation_split <= 0 or validation_split >= 1:
             print(f"Warning: Invalid validation_split ({validation_split}). Training without validation/early stopping.")
             X_train_np, y_train_np = X_data_np, y_data_np
        else:
            try:
                X_train_np, X_val_np, y_train_np, y_val_np = train_test_split(
                    X_data_np, y_data_np, test_size=validation_split, random_state=42, shuffle=True
                )
                print(f"Split data: {len(X_train_np)} train, {len(X_val_np)} validation samples.")
            except Exception as e:
                print(f"Error during train/val split for tech '{tech}': {e}. Training without validation/early stopping.")
                X_train_np, y_train_np = X_data_np, y_data_np # Fallback to using all data for training
        # --- End Split ---
        ...
        try:
            ...
            if X_val_np is not None and y_val_np is not None and len(X_val_np) > 0:
                X_val_tensor = torch.tensor(X_val_np, dtype=torch.float32)
                y_val_tensor = torch.tensor(y_val_np, dtype=torch.long)
                val_dataset = PlacementDataset(X_val_tensor, y_val_tensor)
                val_batch_size = min(batch_size, len(val_dataset)) # Use same batch size or smaller
                if val_batch_size > 0:
                    val_loader = data.DataLoader(
                        val_dataset, batch_size=val_batch_size, shuffle=False, # No shuffle for validation
                        num_workers=min(2, os.cpu_count()), pin_memory=torch.cuda.is_available(),
                    )
                else: # Should not happen if len(X_val_np) > 0, but safety check
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_dir", type=str, default=DEFAULT_MODEL_SAVE_DIR, help="Directory to save best trained models.")
    ...
    parser.add_argument("--es_patience", type=int, default=15, help="Early Stopping: Number of epochs to wait for improvement.") # Increased default
