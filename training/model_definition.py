...
class ModulePlacementCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # --- Block 1 ---
        x = self.conv1(x)
        if torch.isnan(x).any(): print("!!! NaN DETECTED AFTER conv1 !!!")
        x = self.bn1(x)
        if torch.isnan(x).any(): print("!!! NaN DETECTED AFTER bn1 !!!")
        x = self.relu1(x)
        if torch.isnan(x).any(): print("!!! NaN DETECTED AFTER relu1 !!!")

        # --- Block 2 ---
        x = self.conv2(x)
        if torch.isnan(x).any(): print("!!! NaN DETECTED AFTER conv2 !!!")
        x = self.bn2(x)
        if torch.isnan(x).any(): print("!!! NaN DETECTED AFTER bn2 !!!")
        x = self.relu2(x)
        if torch.isnan(x).any(): print("!!! NaN DETECTED AFTER relu2 !!!")

        # --- Block 3 ---
        # <<< Check input to conv3 >>>
        x_before_conv3 = x
        if torch.isnan(x_before_conv3).any():
             print("!!! NaN DETECTED *BEFORE* conv3 !!!") # Should not happen based on logs, but good check
        # <<< End Check >>>
        ...
        if torch.isnan(x).any():
            ...
            print(f"  Input to conv3 min/max/mean: {x_before_conv3.min():.4f} / {x_before_conv3.max():.4f} / {x_before_conv3.mean():.4f}")
            # <<< End Print >>>

        x = self.bn3(x)
        if torch.isnan(x).any(): print("!!! NaN DETECTED AFTER bn3 !!!")
        x = self.relu3(x)
        if torch.isnan(x).any(): print("!!! NaN DETECTED AFTER relu3 !!!")

        # --- Dropout ---
        x = self.dropout(x)
        if torch.isnan(x).any(): print("!!! NaN DETECTED AFTER dropout !!!")

        # --- Final Output Layer ---
        x_placement = self.output_conv(x)
        if torch.isnan(x_placement).any(): print("!!! NaN DETECTED AFTER output_conv !!!")
        ...
        if torch.isnan(x_placement).any():
             ...
             nan_output_shape = (x_placement.shape[0], self.num_output_classes, self.grid_height, self.grid_width)
