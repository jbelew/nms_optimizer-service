...
class TestScEligibleEdgeCases:
    """Tests for edge cases in sc_eligible enforcement."""

    @pytest.fixture
    def grid_with_supercharged(self):
        """Create a small 4x4 grid with supercharged slots at (0,0) and (0,2)."""
        return create_grid(4, 4, supercharged_positions=[(0, 0), (0, 2)])

    @pytest.fixture
    def non_eligible_modules(self):
        """Modules with sc_eligible = False."""
        return [
            {
                "id": "NE1",
    ...
    @pytest.fixture
    def eligible_modules(self):
        """Modules with sc_eligible = True."""
        return [
            {
                "id": "E1",
                "label": "Eligible 1",
    ...
    def test_non_eligible_modules_skip_supercharged(
        self, grid_with_supercharged, non_eligible_modules
    ):
        """Test that place_all_modules_in_empty_slots skips supercharged slots for non-eligible modules."""
        grid = grid_with_supercharged
        result_grid = place_all_modules_in_empty_slots(grid, {}, "standard", "trails", non_eligible_modules)
        ...
        for x, y in [(0, 0), (0, 2)]:  # supercharged positions
            ...
            if cell["module"] is not None:
                ...
                module = next((m for m in non_eligible_modules if m["id"] == module_id), None)
                assert module is None or module.get("sc_eligible", False), \
                    f"Non-eligible module {module_id} placed in supercharged slot ({x}, {y})"

    def test_eligible_modules_prefer_supercharged(
        self, grid_with_supercharged, eligible_modules
    ):
        """Test that place_all_modules_in_empty_slots prefers supercharged slots for eligible modules."""
        grid = grid_with_supercharged
        result_grid = place_all_modules_in_empty_slots(grid, {}, "standard", "trails", eligible_modules)
        ...
        if modules_placed == len(eligible_modules):
            assert supercharged_occupied, "Eligible modules placed but no supercharged slots used"

    def test_refine_placement_rejects_invalid_permutations(
        self, grid_with_supercharged, non_eligible_modules
    ):
        ...
        optimal_grid, best_score = refine_placement(
            ...
            {},
            "trails",
            tech_modules=non_eligible_modules,
        )
            ...
            for y in range(optimal_grid.height):
                for x in range(optimal_grid.width):
                    ...
                    if cell["supercharged"] and cell["module"] is not None:
                        module = next(
                            (m for m in non_eligible_modules if m["id"] == cell["module"]),
                            None,
                        )
                        assert module is None or module.get("sc_eligible", False), \
    ...
    def test_windowing_rejects_all_non_eligible(self):
        ...
        assert result is None, "Windowing should return None when all modules are non-sc_eligible"
    ...
    def test_windowing_accepts_all_eligible(self):
        ...
        assert result is not None, "Windowing should find opportunity when all modules are sc_eligible"
    ...
    def test_core_placement_skips_supercharged_for_non_eligible(self):
        ...
        assert best_pos != (0, 0), "Should not place non-eligible in supercharged slot (0,0)"
        assert best_pos is not None, "Should find a valid placement position"

    def test_mixed_modules_placement(self, grid_with_supercharged):
        ...
        for y in range(result_grid.height):
            for x in range(result_grid.width):
                ...
                if cell["module"] is not None and cell["supercharged"]:
                    ...
                    assert module is not None and module.get("sc_eligible", False), \
                        f"Non-eligible {cell['module']} in supercharged ({x}, {y})"
    ...
    def test_no_supercharged_slots_non_eligible(self):
        ...
        assert placed, "Non-eligible module should be placed when no supercharged slots exist"
        ...
        for y in range(result_grid.height):
            for x in range(result_grid.width):
                ...
                if cell["module"] == "NE1":
                    ...
                    assert cell["supercharged"], "Module should be in supercharged slot (only option)"
                    break

        assert module_placed, "Non-eligible module should be placed when all slots are supercharged"


class TestScEligibleInvariants:
    ...
    def test_invariant_supercharged_vs_eligible(self):
        ...
        assert should_skip, "Skip logic failed for supercharged + non-eligible"
        ...
        assert not should_skip, "Skip logic incorrectly rejects eligible module"
    ...
    def test_invariant_non_supercharged_always_ok(self):
        ...
        for module in [non_eligible, eligible]:
            ...
            assert not should_skip, f"Incorrectly skipped module in non-supercharged slot"
