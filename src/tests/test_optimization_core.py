...
class TestPrepareOptimizationRun(unittest.TestCase):
    ...
    def test_no_modules_returns_error_tuple(self):
        ...
        # Type guard: ensure result is a 4-tuple
        if len(result) == 4:
    ...
    def test_no_empty_active_slots_raises_error(self):
        ...
        with patch("src.optimization.core.get_tech_modules", return_value=[{"id": "m1", "label": "M1"}]):
    ...
    def test_has_empty_active_slots_returns_modules(self):
        ...
        full_modules = [{"id": f"m{i}", "label": f"M{i}"} for i in range(3)]
        partial_modules = [{"id": f"m{i}", "label": f"M{i}"} for i in range(2)]
        ...
        # Type guard: ensure result is a 2-tuple of lists
    ...
    def test_available_modules_filters_correctly(self):
        ...
        with patch("src.optimization.core.get_tech_modules") as mock_get:
            ...
            result = _prepare_optimization_run(grid, {}, "ship", "tech", available_modules=["m1", "m2"])
    ...
    def test_grid_returned_is_copy_not_original(self):
        ...
        # Type guard: ensure result is a 4-tuple (error case)
        if len(result) == 4:
            result_grid, _, _, _ = result  # type: ignore[misc]
...
class TestOptimizeOptimizationFlow(unittest.TestCase):
    ...
    def test_available_supercharged_slot_detection(self):
        ...
        has_available = any(cell["module"] is None and cell["supercharged"] and cell["active"] for cell in window_cells)
...
class TestPartialModuleSetEdgeCases(unittest.TestCase):
    ...
    def test_partial_set_with_empty_list(self):
        ...
        is_partial = (
            available_modules is not None and full_modules is not None and len(tech_modules) < len(full_modules)
        )
...
class TestRefinementStageEdgeCases(unittest.TestCase):
    ...
    def test_no_pattern_or_scanned_opportunity(self):
        ...
        has_opportunity = pattern_opportunity is not None or scanned_opportunity is not None
...
class TestSuperchargedWindowDetection(unittest.TestCase):
    ...
    def test_supercharged_detection_inactive_cells(self):
        ...
        cell_inactive_sc = {"active": False, "supercharged": True, "module": None}

        # Should not count as available
        has_available = (
            cell_inactive_sc["active"] and cell_inactive_sc["supercharged"] and cell_inactive_sc["module"] is None
        )
        self.assertFalse(has_available)
    ...
    def test_supercharged_with_existing_module(self):
        ...
        available = [c for c in cells if c["active"] and c["supercharged"] and c["module"] is None]
