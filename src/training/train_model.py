...
class PlacementDataset(data.Dataset):
    def __init__(self, X_supercharge, y, X_inactive_mask=None):
    ...
    def __getitem__(self, idx):
        ...
        input_tensor = torch.stack(
            [
                ...
                torch.tensor(x_inactive, dtype=torch.float32),
            ],
            dim=0,
        )
...
def train_model(
    ...
    print(
        f"  Early Stopping: Patience={early_stopping_patience}, Metric='{early_stopping_metric}'"
    )
    print(
        f"  Grid Dimensions: {grid_height}x{grid_width}"
    )  # <<< Use passed-in dynamic dimensions
    print(f"  Num Output Classes: {num_output_classes}")
    print(f"  Epochs: {num_epochs}, LR: {learning_rate}, WD: {weight_decay}")
    print(
        f"  Scheduler: ReduceLROnPlateau, Factor: {scheduler_factor}, Patience: {scheduler_patience}"
    )
    ...
    if torchmetrics:
        try:
            val_accuracy = torchmetrics.Accuracy(
                task="multiclass", num_classes=num_output_classes, average="macro"
            ).to(device)
            ...
            print(
                f"Warning: Failed to initialize torchmetrics: {e}. Validation metrics will be unavailable."
            )
            val_accuracy = None
            val_iou = None
    else:
        print(
            "Warning: torchmetrics not installed. Validation accuracy and mIoU will not be calculated."
        )

    # --- Early Stopping Initialization ---
    if early_stopping_metric == "val_miou" and not metrics_available:
        print(
            "Warning: Cannot monitor 'val_miou' as torchmetrics failed or is not installed. Defaulting to 'val_loss'."
        )
    ...
    for epoch in range(num_epochs):
        ...
        for i, (inputs, targets_placement) in enumerate(train_loader):
            ...
            loss_placement = criterion_placement(
                outputs_placement, targets_placement.long()
            )
            loss = loss_placement
            ...
            if torch.isnan(loss):
                ...
                print(
                    f"  Output Logits min/max/mean: {outputs_placement.min():.4f} / {outputs_placement.max():.4f} / {outputs_placement.mean():.4f}"
                )
            ...
            if torch.isnan(loss):
                print(
                    f"ERROR: NaN loss detected at Epoch {epoch + 1}, Batch {i}. Stopping training."
                )
        ...
        avg_train_loss = (
            running_train_loss / len(train_loader) if len(train_loader) > 0 else 0.0
        )
        ...
        if val_loader:
            ...
            with torch.no_grad():
                for i, (inputs, targets_placement) in enumerate(val_loader):
                    inputs, targets_placement = (
                        inputs.to(device),
                        targets_placement.to(device),
                    )
                    outputs_placement = model(inputs)
                    loss_placement = criterion_placement(
                        outputs_placement, targets_placement.long()
                    ...
                    if torch.isnan(loss_placement):
                        print(
                            f"ERROR: NaN validation loss detected at Epoch {epoch + 1}. Stopping training."
                        )
            ...
            avg_val_loss = (
                running_val_loss / len(val_loader) if len(val_loader) > 0 else 0.0
            )
            epoch_val_accuracy = (
                val_accuracy.compute() if metrics_available and val_accuracy else 0.0
            )
        ...
        if current_metric_value is not None:  # Check if metric was calculated
            is_nan_check = (
                torch.isnan(current_metric_value)
                if isinstance(current_metric_value, torch.Tensor)
                else np.isnan(current_metric_value)
            )
        ...
        if (
            val_loader
            and current_metric_value is not None
            and not is_nan_check_scheduler
        ):
            scheduler.step(current_metric_value)
    ...
    if not early_stop_triggered:
        ...
        if not val_loader:
            try:
                print(
                    f"Saving final model state (no validation performed) to {model_save_path}"
                )
                model.to("cpu")
                torch.save(model.state_dict(), model_save_path)
                model.to(device)
    ...
    if val_loader:
        try:
            ...
            best_metric_str = (
                f"{best_metric_value_float:.4f}"
                if not is_best_invalid
                else "N/A (or NaN/Inf)"
            )
            # <<< End conversion >>>

            print(
                f"Loading best model state from {model_save_path} (Metric: {early_stopping_metric}, Value: {best_metric_str})"
            )  # Use best_metric_str
            if os.path.exists(model_save_path):
                ...
                print(
                    f"Warning: Best model file {model_save_path} not found. Returning last state."
                )
        except Exception as e:
            # <<< Modify warning message slightly for clarity >>>
            print(
                f"Warning: Could not load or process best model state after training: {e}. Returning last state."
            )
...
def run_training_from_files(
    ...
    try:
        ...
        if (
            ...
            raise KeyError(
                f"Ship '{ship}' or its 'types' dictionary not found/invalid."
            )
    ...
    if specific_techs_to_train:
        print(
            f"Attempting to train specific techs for ship '{ship}': {specific_techs_to_train}"
        )
        ...
        for tech_key_input in specific_techs_to_train:
            if tech_key_input in all_valid_ship_techs:
                ...
                print(
                    f"  Warning: Specified tech key '{tech_key_input}' not found for ship '{ship}'. Skipping."
                )
        if not tech_keys_to_process:
            print(
                f"Error: None of the specified tech keys were valid for ship '{ship}'. Aborting."
            )
            return
        print(
            f"  Will train the following valid specific techs: {tech_keys_to_process}"
        )

    elif tech_category_to_train:
        print(
            f"Attempting to train techs in category '{tech_category_to_train}' for ship '{ship}'."
        )
        category_data = ship_data["types"].get(tech_category_to_train)
        if not category_data or not isinstance(category_data, list):
            print(
                f"  Error: Category '{tech_category_to_train}' not found or invalid for ship '{ship}'."
            )
            return
        tech_keys_to_process = [
            t_data["key"]
            for t_data in category_data
            if isinstance(t_data, dict) and "key" in t_data
        ]
        if not tech_keys_to_process:
            print(
                f"  Warning: No valid tech keys found in category '{tech_category_to_train}'. Skipping category."
            )
        else:
            print(
                f"  Will train techs in category '{tech_category_to_train}': {tech_keys_to_process}"
            )
    else:  # Neither specific techs nor category specified - train all techs for the ship
        print(
            f"No specific techs or category specified. Planning to train all techs for ship '{ship}'."
        )
        for cat_data_list in ship_data["types"].values():
            for tech_item in cat_data_list:
                if isinstance(tech_item, dict) and "key" in tech_item:
    ...
    print(
        f"Looking for data files in subdirectories under: {os.path.abspath(base_data_source_dir)}"
    )
    trained_models = {}
    ...
    for tech in tech_keys_to_process:
        ...
        if num_output_classes <= 1:
            print(
                f"Skipping tech '{tech}': Not enough output classes ({num_output_classes})."
            )
            continue
        print(
            f"  Expected number of output classes (incl. background): {num_output_classes}"
        )
        ...
        if not data_files:
            print(
                f"Warning: No data files found matching pattern '{file_pattern}'. Skipping tech '{tech}'."
            )
            continue
        print(
            f"Found {len(data_files)} data files for tech '{tech}'. Loading and validating..."
        )  # <<< Updated print
        ...
        for filepath in data_files:
            try:
                with np.load(filepath) as npz_file:
                    if (
                        "X_supercharge" not in npz_file
                        or "X_inactive_mask" not in npz_file
                        or "y" not in npz_file
                    ):
                        print(
                            f"Warning: Required keys not found in {filepath}. Skipping file."
                        )
                    ...
                    valid_file = True
                    if (
                        len(x_sc_batch.shape) < 3
                        or len(x_inactive_batch.shape) < 3
                        or len(y_batch.shape) < 3
                    ):
                        print(
                            f"Warning: Unexpected array dimensions in {filepath}. Skipping file."
                        )
                        valid_file = False
                    # <<< Use dynamic dimensions for shape check >>>
                        ...
                        x_sc_batch.shape[0]
                        == x_inactive_batch.shape[0]
                        == y_batch.shape[0]
                    ):
                        print(
                            f"Warning: Sample count mismatch in {filepath}. Skipping file."
                        )
                        valid_file = False
                    # <<< Add Label Validation >>>
                    elif np.any(y_batch < 0):
                        print(
                            f"Warning: Found negative labels in 'y' array in {filepath}. Skipping file."
                        )
                        valid_file = False
                    elif np.any(y_batch >= num_output_classes):
                        max_label = np.max(y_batch)
                        print(
                            f"Warning: Found labels >= num_output_classes ({num_output_classes}) in 'y' array (max label: {max_label}) in {filepath}. Skipping file."
                        )
                        valid_file = False
                ...
                print(
                    f"Error loading or validating data from {filepath}: {e}. Skipping file."
                )
                invalid_files_count += 1
                continue
        ...
        if invalid_files_count > 0:
            print(
                f"  Skipped {invalid_files_count} invalid or problematic data files for tech '{tech}'."
            )

        if not all_X_supercharge_data:
            print(
                f"Warning: No valid data could be loaded for tech '{tech}' after validation. Skipping."
            )
            continue
        ...
        try:
            ...
            print(
                f"Loaded and concatenated {total_loaded_samples} total valid samples for tech '{tech}' in {load_time:.2f}s."
            )
        except Exception as e:
        ...
        if len(X_supercharge_data_np) < 2:
            print(
                f"Warning: Not enough samples ({len(X_supercharge_data_np)}) for tech '{tech}' for validation."
            )
            ...
            print(
                f"Warning: Invalid validation_split ({validation_split}). Training without validation."
            )
            X_train_sc_np, X_train_in_np, y_train_np = (
                X_supercharge_data_np,
                X_inactive_mask_data_np,
            ...
            try:
                ...
                print(
                    f"Split data: {len(X_train_sc_np)} train, {len(X_val_sc_np)} validation samples."
                )
            except Exception as e:
                print(
                    f"Error during train/val split for tech '{tech}': {e}. Training without validation."
                )
                X_train_sc_np, X_train_in_np, y_train_np = (
                    X_supercharge_data_np,
                    X_inactive_mask_data_np,
        ...
        if y_train_np is not None and y_train_np.size > 0:
            ...
            if np.any(class_counts == 0):
                ...
                print(
                    f"  Warning: Classes {zero_count_classes} not found in training data for {tech}. Assigning weight 1.0."
                )
            ...
            print(
                "Warning: y_train_np not available or empty. Cannot calculate class weights."
            )
        # --- End Calculate Class Weights ---

        # --- 3. Prepare Datasets and DataLoaders ---
        try:
            train_dataset = PlacementDataset(
                X_train_sc_np, y_train_np, X_inactive_mask=X_train_in_np
            )
            if (
                X_val_sc_np is not None
                and X_val_in_np is not None
                and y_val_np is not None
                and len(X_val_sc_np) > 0
            ):
                val_dataset = PlacementDataset(
                    X_val_sc_np, y_val_np, X_inactive_mask=X_val_in_np
                )
                ...
                if val_batch_size > 0:
                    ...
                    print(
                        f"Warning: Validation dataset for tech '{tech}' is empty. Skipping validation."
                    )
                    val_loader = None
            else:
                print(
                    f"Info: No validation data available for tech '{tech}'. Skipping validation."
                )
                val_loader = None
        except Exception as e:
        ...
        if not can_early_stop:
            print(
                "Info: No validation loader available. Early stopping disabled for this tech."
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train NMS Optimizer models with validation and early stopping."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Technology category to train (optional, used if --techs is not provided).",
    )
    parser.add_argument(
        "--techs",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="ReduceLROnPlateau: Factor by which the LR is reduced.",
    )
    parser.add_argument(
        "--scheduler_patience",
        type=int,
        default=8,
        help="ReduceLROnPlateau: Number of epochs with no improvement after which LR is reduced.",
    )
    parser.add_argument(
        "--log_dir",
        type=str,
        default=DEFAULT_LOG_DIR,
        help="Base directory for TensorBoard logs (ship/tech subdirs will be created).",
    )
    parser.add_argument(
        "--model_dir",
        type=str,
        default=DEFAULT_MODEL_SAVE_DIR,
        help="Base directory to save best trained models.",
    )
    parser.add_argument(
        "--data_source_dir",
        type=str,
        default=DEFAULT_DATA_SOURCE_DIR,
        help="Base directory containing generated .npz data files (expects ship/tech subdirs).",
    )
    parser.add_argument(
        "--val_split",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Early Stopping: Number of epochs to wait for improvement.",
    )
    parser.add_argument(
        "--es_metric",
        type=str,
    ...
    if args.techs:
        specific_techs_list = [
            tech.strip() for tech in args.techs.split(",") if tech.strip()
        ]
    ...
    run_training_from_files(
        ship=config["ship"],
        tech_category_to_train=config[
            "tech_category_to_train"
        ],  # <<< Pass the value from args
    ...
    print(
        f"Best models saved directly in: {os.path.abspath(config['base_model_save_dir'])}"
    )
