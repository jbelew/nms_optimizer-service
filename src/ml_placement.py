...
def ml_placement(
    ...
    if not os.path.exists(model_path):
        ...
        logging.warning(f"WARNING -- ML Placement: Model file for ('{filename_ship_key}', '{filename_tech_key}') not found at '{model_path}'. Cannot use ML.")
        return None, 0.0
    ...
    training_modules_list = get_tech_modules_for_training(modules_for_training_defs, module_def_ship_key, module_def_tech_key)

    if not training_modules_list:
        # Use f-string for cleaner formatting
        # Important failure condition
        logging.error(f"ERROR -- ML Placement: No TRAINING modules found for Module Def keys ('{module_def_ship_key}', '{module_def_tech_key}'). Cannot define model outputs.")
        return None, 0.0
